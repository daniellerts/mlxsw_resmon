...
static int resmon_dl_netlink_init(struct nl_sock *sk, int *family, char **error)
{
	...
	err = genl_connect(sk);
	if (err) {
		asprintf(error, "Failed to connect socket");
		goto err_genl_connect;
	}

	err = nl_socket_set_nonblocking(sk);
	if (err) {
		asprintf(error, "Failed to set socket nonblocking");
		goto err_genl_connect;
	}

	*family = genl_ctrl_resolve(sk, "devlink");
	if (*family < 0) {
		asprintf(error, "Failed to resolve ID of \"devlink\" family");
...
static int resmon_dl_dev_info_parser(struct nl_msg *msg, void *arg)
{
	...
	if (attrs[DEVLINK_ATTR_BUS_NAME] &&
		...
		if (strcmp(attr_driver_name, "mlxsw_spectrum") == 0) {
				     ...
				     char **error)
{
	...
	if (err < 0) {
		asprintf(error, "Failed to send devlink get command");
		return err;
	}
	...
	if (err < 0) {
		asprintf(error, "Failed to set devlink info parser");
		return err;
	}

	err = nl_recvmsgs(sk, cb);
	if (err < 0 || args.err) {
		asprintf(error, "Failed to recieve messages from netlink");
		return err;
	}
					  ...
					  uint64_t *size, char **error)
{
	...
	msg = nlmsg_alloc();
	if (!msg) {
		asprintf(error, "Failed to allocate netlink message");
		return -1;
	}
	...
	if (err < 0) {
		asprintf(error, "Failed to send devlink resource get command");
		return err;
	}

	len = nl_recv(sk, &nla, &buf, NULL);
	if (len < 0) {
		asprintf(error, "Failed to recieve message");
		return -1;
	}
...
int resmon_dl_get_kvd_size(uint64_t *size, char **error)
{
	...
	sk = nl_socket_alloc();
	if (!sk) {
		asprintf(error, "Failed to allocate data socket");
		return -1;
	}
	...
	if (err < 0) {
		asprintf(error, "Failed to open netlink socket");
		return -1;
	}

	err = resmon_dl_netlink_get_dev(sk, family, &busname, &devname, error);
	if (err < 0) {
		asprintf(error, "Failed to get devlink dev from netlink");
		return -1;
	}
	...
	err = resmon_dl_netlink_get_kvd_size(sk, family, busname, devname,
					     size, error);
	if (err < 0) {
		asprintf(error, "Failed to get devlink resource size from netlink");
		return -1;
	}
